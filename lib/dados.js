...
const ARQUIVOS = {
  ...
  laboratorios: 'data/laboratorios.json',
  pendentes: 'data/laboratorios-pendentes.json'
};

const cache = {
  noticias: { dados: null, em: 0 },
  webinars: { dados: null, em: 0 },
  laboratorios: { dados: null, em: 0 },
  pendentes: { dados: null, em: 0 }
};
...
const getLaboratorios = () => ler('laboratorios');
const saveLaboratorios = (dados) => salvar('laboratorios', dados);

// Fila de cadastros enviados pelo site, à espera de aprovação no painel.
// Ficam em arquivo separado para não haver risco de um pendente aparecer
// no mapa por engano.
const getPendentes = () => ler('pendentes');
const savePendentes = (dados) => salvar('pendentes', dados);
...
/**
 * Aceita só endereços http(s) — o campo do formulário é livre e recebe de tudo,
 * de "não temos" a e-mail. Devolve null quando não for um link utilizável.
 */
function urlOuNulo(valor) {
